...
if ($db->isUserExisted($forgotpassword)) {
	...
	$search_ride = $db->getDevice($uid);
	if($search_ride){
		$response["success"] = 1;
		$response["search_ride"]["uid"] = $search_ride["uid"];
		$response["search_ride"]["latitude"] = $search_ride["latitude"];
		$response["search_ride"]["longitude"] = $search_ride["longitude"];
		echo json_encode($response);
		
	} else{
		//search_ride failed to store
		$response["error"]=1;
		$response["error_msg"] = "JSON Error occured in search ride";
		echo json_encode($response);
	}	
